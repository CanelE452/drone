...
## 5. Phase 5 — 실기체 전환 (무시뮬 안전 프로토콜)

> **결정 (2026-06-10)**: Sphinx 검증은 생략한다. 따라서 `real.py`의 Olympe 호출부가
> **처음 실행되는 장소가 실제 비행**이 된다. Sphinx가 해주던 일("버그가 드러나도 비용 0")을,
> **첫 명령들을 버그가 있어도 안전한 형태로 설계**하는 것으로 대체한다.
> 부호 반전 같은 치명적 버그가 1.2m 호버에서 30cm 명령으로 드러나면 "어 반대로 가네" 하고
> 착륙해서 고치면 끝 — 이게 이 프로토콜의 핵심 아이디어다.
>
> **공통 규칙**: 모든 단계 사이에 착륙한다. 모든 단계에서 긴급착륙 버튼을 즉시 누를 수 있는
> 상태를 유지한다(손은 키보드 Space 또는 마우스 위). 한 단계라도 이상하면 그 자리에서 중단·착륙.

### 5.0 0단계 — 지상 게이트 (이륙 없음, 드론은 책상 위)
- `DRONE_IP=192.168.42.1` (실기체 WiFi) 또는 SkyController 경유로 연결.
- 확인 항목 (전부 통과해야 다음 단계):
  - `connect()` 성공.
  - 텔레메트리 수신 + **값이 말이 되는지**: 배터리 %가 실제와 일치, 자세(roll/pitch/yaw)가 정지 상태에 맞는 값.
  - Link quality rx > 0 가 **5분 유지** (연결 끊김/패킷 손실 없음).
- 여기서 연결 계층 버그(IP, 인증, get_state 키 오류, `_refresh` 예외)는 전부 잡힌다. **이륙 안 함.**

### 5.1 1단계 — 이륙/착륙만 (이동 명령 없음)
- `takeoff()` → 호버 10초 → `land()`.
- 확인: `TakeOff >> FlyingStateChanged(state="hovering")` 체이닝이 실전에서 통하는지, 착륙까지 깔끔한지.
- 이동(moveBy)은 **아직 안 보냄**.

### 5.2 2단계 — 부호 검증 비행 (단 1회 명령, 버그가 있어도 안전하게 설계)
- **고도 부호**: 호버 고도 **1.2m**에서 "위로 30cm" **단 한 번**.
  - 부호가 맞으면 1.5m로 상승, NED 부호 버그면 0.9m로 하강 — **둘 다 안전한 결과**가 되도록 시작 고도를 잡은 것.
  - 결과를 텔레메트리 고도로 확인. 반대로 가면 즉시 착륙 → `real.move_cm`의 `dZ = -up_cm/100` 부호 점검.
- **수평 부호**: 사방 **3m 이상** 빈 공간에서 "앞으로 30cm" 한 번 → 실제 이동 방향이 전방과 일치하는지.
- 둘 다 일치 확인 후에만 3단계로.

### 5.3 3단계 — 전체 시나리오 (2단계 통과 후에만)
- 스텝 이동(전/후/좌/우/상/하) → 고도 설정(`go_to_altitude`) → 초기 위치 복귀(`return_home_initial`) 풀 테스트.
- 이 단계에서 `moveByEnd` 실측 누적(real.py TODO)을 실제 값으로 채우고 복귀 정확도 확인.

### 5.4 고급 기능 (3단계까지 안정된 뒤)
1. PCMD 기반 연속 조종 (키 홀드 → 25ms 주기 PCMD 전송, 키 릴리즈 → 0 전송).
2. 게임패드 지원 (Gamepad API).
3. 영상 스트리밍 (4.3, PDrAW→MJPEG).

---

## 6. 작업 순서 요약 (CLI 체크리스트)

- [x] 1. 환경 실측 — conda env `olympe`(3.10), parrot-olympe 8.4.0 import 성공 확인
- [x] 2. 프로젝트 구조 생성 (`anafi-control/`)
- [x] 3. ~~Sphinx 설치~~ → **생략 결정**. real.py 첫 검증은 실기체 5.0 지상 게이트에서.
- [x] 4. `controller`(interface/mock/real) 구현
- [x] 5. `move_cm` / `go_to_altitude` / `return_home_initial` 구현 (mock 검증 통과)
- [x] 6. `telemetry.py` + 서버 10Hz broadcast 구현
- [x] 7. FastAPI + WebSocket 서버 + index.html UI 구현
- [x] 8. **Mock 통합 테스트 통과** (WebSocket 왕복: 이륙→hovering→move→실측 반영, ±10% drift)
- [ ] 9. GitHub push (완료) → 노트북 WSL 세팅(clone + env 생성 + mock 기동 확인)
- [ ] 10. 실기체 무시뮬 안전 프로토콜 (Phase 5: 0단계 지상게이트 → 1 이착륙 → 2 부호검증 → 3 전체)
- [ ] 11. (선택) PCMD 연속조종, 게임패드, 영상 스트리밍
